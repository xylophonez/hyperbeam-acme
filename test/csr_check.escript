#!/usr/bin/env escript
%%% Offline anchor for lib_acme_csr: generate a wildcard CSR and verify it with
...
main(_) ->
    ...
    {Der, Key} = lib_acme_csr:generate(Names),
    ok = file:write_file("/tmp/csr_check.der", Der),
    ok = file:write_file("/tmp/csr_check_key.pem", lib_acme_csr:key_to_pem(Key)),
